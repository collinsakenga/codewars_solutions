def smallest(n):
    ...
    for i in range(1, len(temp)):
        ...
        if smallest==int(temp[i]): index=i
    if int(temp[1])==smallest:
        if temp[1:].count(str(smallest))>=2 and int(temp[0])<=int(temp[2]):
            res="".join(temp[index])+"".join(temp[0:index])+"".join(temp[index+1:])
            while True:
    ...
    while True:
        if int(temp[index-1])!=int(temp[index]):
            break
        index-=1
    res="".join(temp[:index2])+temp[index]+"".join(temp[index2:index])+"".join(temp[index+1:])
