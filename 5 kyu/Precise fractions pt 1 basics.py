...
class Fraction(object):      
    ...
    def __str__(self):
        self.reduce();
        if not self.up: return f"{self.quotient}" if self.flag else f"-{self.quotient}"
        if self.quotient: return f"{self.quotient} {self.up}/{self.down}" if self.flag else f"-{self.quotient} {self.up}/{self.down}"
    ...
    def reduce(self):
        self.flag=False if abs(self.up)//self.up*abs(self.down)//self.down==-1 else True
        self.up=abs(self.up)
