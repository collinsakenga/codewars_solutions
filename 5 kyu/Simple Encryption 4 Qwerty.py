...
def cipher(text, encryptKey, flag):
    ...
    for i in text:
        if i in key1:
            ...
            res+=key1_upper[(key1_upper.index(i)+flag*(encryptKey//100))%len(key1_upper)]
        elif i in key2_upper:
            res+=key2_upper[(key2_upper.index(i)+flag*(encryptKey//10%10))%len(key2_upper)]
        elif i in key3_upper:
            res+=key3_upper[(key3_upper.index(i)+flag*(encryptKey%10))%len(key3_upper)]
        else:
            res+=i
    return res