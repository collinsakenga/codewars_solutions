def to_utf8_binary(string):
    ...
    for i in string:
        if ord(i)<128:
            ...
            start="110" if ord(i)<2048 else "1110" if ord(i)<65536 else "11110"
...
def from_utf8_binary(bitstring):
    ...
    while bitstring:
        if bitstring[:5]=="11110":
            res+=chr(int(bitstring[5:8]+bitstring[10:16]+bitstring[18:24]+bitstring[26:32], 2))
            bitstring=bitstring[32:]
        elif bitstring[:4]=="1110":
            res+=chr(int(bitstring[4:8]+bitstring[10:16]+bitstring[18:24], 2))
