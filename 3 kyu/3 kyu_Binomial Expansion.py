...
def expand(expr):
    ...
    plus_minus = "+" if (flag1 and flag2) else "-" if (
        not flag1 and not flag2) else ""
    if (not flag1 and not flag2 and start % 2 == 0):
        plus_minus = "+"
    ...
    while start >= 0:
        product = combination(
            rec, start)*coefficient[0]**(start)*coefficient[1]**(rec-start)
        ...
        if plus_minus:
            ...
            temp = "+" if (not flag1 and start % 2 ==
                           0) else "+" if (not flag2 and (rec-start) % 2 == 0) else "-"
