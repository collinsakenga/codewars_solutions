    ...
    def evaluate(self, string):
        ...
        for cal in "/*-+":
            ...
            for i in range(0, len(temp)-1):
                ...
                if not temp[i]:
                    continue
                digit1 = int(self.extract_right(temp[i], cal)) if "." not in self.extract_right(
                    temp[i], cal) else float(self.extract_right(temp[i], cal))
                digit2 = int(self.extract_left(temp[i+1], cal)) if "." not in self.extract_left(
                    temp[i+1], cal) else float(self.extract_left(temp[i+1], cal))
